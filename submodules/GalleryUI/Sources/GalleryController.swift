    ...
    present: @escaping (ViewController, Any?) -> Void) -> GalleryItem?
{
    ...
    if let image = media as? TelegramMediaImage {
        if let file = image.video {
            let captureProtected = message.isCopyProtected() || message.darkGramTreatAsProtectedMedia || message.darkGramIsViewOnceProtected || message.paidContent != nil || peerIsCopyProtected
